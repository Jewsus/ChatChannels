...
namespace ChatChannels
{
    public class Config
    {
        public static string ConfigPath = Path.Combine(TShock.SavePath, "Channels", "ChatChannelsConfig.json");
        ...
        public Config Read(TSPlayer ts = null)
        {
            ...
            catch (Exception ex)
            {
                if (ts == null)
                {
                    Console.WriteLine("[Channels] an error has occurred while reading the config file! See below for more info:");
                    Console.WriteLine(ex.ToString());
                }
                else
                    ts.SendErrorMessage("[Channels] There was an error reloading the config file, check the console for more info!");
                return this;
            }
        }
